...
Future<String> generateTextForElement<T>(
  GeneratorForAnnotation<T> generator,
  LibraryReader libraryReader,
  String name,
) async {
  final elements = libraryReader.allElements.where((e) => e.name == name).toList();
  ...
  if (elements.length == 1) {
    ...
    final rootProperties = elements.whereType<PropertyInducingElement>().toList();
    ...
    final annotationFromTestLib = element.metadata
        ...
        .where((obj) {
          if (obj.type is InterfaceType) {
            ...
            return uri.isScheme('package') && uri.pathSegments.first == testPackageName;
          }
  ...
  final val = await generatedStream.join('\n\n');
