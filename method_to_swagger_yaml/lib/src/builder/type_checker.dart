...
abstract class TypeChecker {
  ...
  bool hasAnnotationOfExact(Element element, {bool throwOnUnresolved = true}) =>
      firstAnnotationOfExact(element, throwOnUnresolved: throwOnUnresolved) != null;
  ...
  bool isAssignableFrom(Element element) =>
      isExactly(element) || (element is InterfaceElement && element.allSupertypes.any(isExactlyType));
...
class _LibraryTypeChecker extends TypeChecker {
  ...
  @override
  bool isExactly(Element element) => element is InterfaceElement && element == _type.element;
...
class _MirrorTypeChecker extends TypeChecker {
  static Uri _uriOf(ClassMirror mirror) => normalizeUrl((mirror.owner as LibraryMirror).uri)
...
class _UriTypeChecker extends TypeChecker {
  ...
  bool hasSameUrl(dynamic url) =>
      uri.toString() == (url is String ? url : normalizeUrl(url as Uri).toString());
...
class UnresolvedAnnotationException implements Exception {
  ...
  static SourceSpan? _findSpan(
    ...
    try {
      final parsedLibrary = annotatedElement.session!.getParsedLibraryByElement(annotatedElement.library!)
